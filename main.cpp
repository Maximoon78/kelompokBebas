...
int maxy=600, count=0, maxx=500, n=1, m=1,r,q;
...
void counting()
{
sprintf(str,"Anak Panah Keluar:%d",count);
...
sprintf(str,"Panah keluar:%d",count);
bitmap_output(5,250,str,GLUT_BITMAP_TIMES_ROMAN_24);

glutIdleFunc(NULL);
}
else if(count>=25)
{
sprintf(str,"Whoops, anda kalah!",count);
...
bitmap_output(0,40,"Kelompok Bebas",GLUT_BITMAP_TIMES_ROMAN_24);
counting();
...
//to move the arrow left when ‘e’ pressed
void id1()
{
x+=m;
disa();
...
if(k=='E')
{
maxy=600; count=0; maxx=500; n=1; m=1;
...
glutCreateWindow("Kelompok Bebas");
sub_menu=glutCreateMenu(demo_menu);
glutAddMenuEntry("                                            ||HOW TO PLAY||",5);
glutAddMenuEntry("'E' untuk bergerak ke kanan",6);
glutAddMenuEntry("Ada 10 kotak yang harus ditembak",7);
glutAddMenuEntry("Anda kalah jika jumlah panah berjumlah 25 buah anak panah",8);
glutAddMenuEntry("Makannya harus kena semua kotaknya",9);
glutCreateMenu(game);
glutAddSubMenu("Game dibuat oleh Gerald Marihot dan Valen Novandi",sub_menu);
